...
/**
 * Post-upload processing - simplified for local research tool
 */
@Component
    ...
    public void file_task(ArticleInfo articleInfo) {
        ...
        if(ifsuccess) {
            ...
            if (!new File(cajpath).exists() && !new File(pdfpath).exists()) {
                // Document file upload - use Qwen document understanding
                try {
                    System.out.println("将文档上传给Qwen模型,会花几十秒");
                    LogUtil_AfterUpload.log("将文档上传给Qwen模型,会花几十秒");
                    ...
                    System.out.println("Qwen模型处理完毕,已经结果存入数据库,使用的模型是" + Config.QWEN_DOC_MODEL);
                    LogUtil_AfterUpload.log("Qwen模型处理完毕,已经结果存入数据库,使用的模型是" + Config.QWEN_DOC_MODEL);
                } catch (Exception e) {
                    System.out.println("调用失败" + e.getMessage());
                    LogUtil_AfterUpload.log("调用失败" + e.getMessage());
                }
            } else {
                // Text upload - use Qwen text generation (simplified, no model comparison)
                try {
                    String content = new String(Files.readAllBytes(Paths.get(txtpath)));
                    System.out.println("将文本上传给Qwen模型,会花几十秒，当前论文字符长度: " + content.length());
                    LogUtil_AfterUpload.log("将文本上传给Qwen模型,会花几十秒，当前论文字符长度: " + content.length());
                    
                    String qwenres = QwenChat(Config.QWEN_MODEL, content + Config.JSON + s);
                    saveSummary(Config.QWEN_MODEL, articleInfo.getTitle(), qwenres, "0");
                    
                    System.out.println("Qwen模型处理完毕,已将结果存入数据库,使用的模型是:" + Config.QWEN_MODEL);
                    LogUtil_AfterUpload.log("Qwen模型处理完毕,已将结果存入数据库,使用的模型是:" + Config.QWEN_MODEL);
                } catch (IOException e) {
                    System.out.println("调用失败" + e.getMessage());
                    LogUtil_AfterUpload.log("调用失败" + e.getMessage());
                } catch (Exception e) {
